...
function SearchByAi({ onClose }) {
  ...
  const { setAiSearchResults, setSearchMode } = useContext(searcContext);

  // 🔥 Fetch products here inside SearchByAi
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const matchedProducts = allProducts
        ...
        .filter((product) => product.similarity > 0.45)
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-6 bg-white rounded-2xl shadow-sm border max-w-md mx-auto">
      <h2 className="text-2xl font-semibold">AI Image Search</h2>
