...
import { useDispatch, useSelector } from 'react-redux';
import { motion, AnimatePresence } from 'framer-motion';
import { FaCartPlus, FaHistory, FaSignOutAlt, FaBars, FaCamera } from 'react-icons/fa';
...
function Header() {
  ...
  // Click on search input → Show ProductList with filters
  const handleSearchFocus = () => {
    resetToNormalMode();        // This will trigger ProductList in Home
  };
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:block flex-1 max-w-xl mx-8">
              <input
                ...
                className="w-full px-5 py-2.5 bg-gray-800 border border-gray-700 rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-500 cursor-pointer"
              />
            </div>
            ...
            <button 
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)} 
              className="md:hidden p-3 text-white hover:bg-gray-800 rounded-lg"
            >
              <FaBars size={28} />
            </button>
          </div>
