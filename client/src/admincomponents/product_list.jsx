...
function ProductList() {
  ...
  const enableAiSearch = async (id) => {
    try {
      setLoadingId(id); // 👈 start loading for this button

      const res = await generateAiSearch(id);

      console.log("response from generate ai : ", res);

      toast.success(res?.data?.message || "AI Enabled");

      fetchProducts();
    } catch (error) {
      console.error(error);
      toast.error("Error enabling AI search");
    } finally {
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        ...
                        <button
                          ...
                          disabled={product?.embedding?.length > 0 || loadingId === product._id}
